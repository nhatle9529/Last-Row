...
Module for calculating a Pitch Control surface using Last row's tracking data.
...
The code was inspired and based on the work from LaurieOnTracking (with Metrica's data), you can find his work on this repo :
...
generate_pitch_control_for_moment(): this function evaluates pitch control surface over the entire field at the moment
of the given event
...
def initialise_players(frame_df,teamname,params):
    """
    ...
    frame_df : tracking Dataframe for a given play
    teamname: team name "defense" or "attack"
    ...
    for p in player_ids:
        ...
        team_player = player(p,frame_df,teamname,params)
...
class player(object):
    ...
    frame_df: tracking data for team at a given frame
    ...
    def __init__(self,pid,frame_df,teamname,params):
        self.id = pid
        ...
        self.get_position(frame_df)
        self.get_velocity(frame_df)
        self.PPCF = 0. # initialise this for later
        
    def get_position(self,frame_df):
        player_df = frame_df[frame_df['player'] == self.id]
        self.position = np.array( [ player_df.iloc[0]['x_m'], player_df.iloc[0]['y_m'] ] )
        self.inframe = not np.any( np.isnan(self.position) )
        
    def get_velocity(self,frame_df):
        player_df = frame_df[frame_df['player'] == self.id]
        self.velocity = np.array( [ player_df.iloc[0]['vx_m'], player_df.iloc[0]['vy_m'] ] )
...
def default_model_params(time_to_control_veto=3):
    ...
    params['int_dt'] = 0.05 # integration timestep (dt)
...
def generate_pitch_control_for_frame(frame, tracking_attack, tracking_defense, params, field_dimen = (106.,68.,), n_grid_cells_x = 50):
    ...
    ball_start_pos = np.array([list(tracking_attack[(tracking_attack['frame'] == frame)]['ball_x_m'])[0],list(tracking_attack[(tracking_attack['frame'] == frame)]['ball_y_m'])[0]])
    ...
    defending_players = initialise_players(tracking_defense[tracking_defense['frame']==frame],'defense',params)
    attacking_players = initialise_players(tracking_attack[tracking_attack['frame']==frame],'attack',params)
